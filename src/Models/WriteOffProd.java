...
class WriteOffProd extends JFrame {
	...
	private void okbtn2ActionPerformed() {
		try {
			...
			pr.execute();
			pr = cn.prepareStatement(
					"insert into \"Writen_Off_Product\" (\"idWrittenOffProduct\",\"reason\",\"idProduct\",\"amount\")\n"
							+ "Values("+id+","+reasontextArea.getText()+","+codetextField.getText()+","+counttextField.getText()+")");
	...
	private void initComponents() {
		...
		contentPane.setLayout(new MigLayout("hidemode 3",
				// columns
				"[4,fill]" + "[87,grow,fill]" + "[165,grow,fill]",
				// rows
				"[]" + "[40,grow,fill]" + "[42,grow,fill]" + "[46,grow,fill]" + "[95,grow,fill]" + "[39,grow,fill]"));
		...
		// ======== writeoffdialog ========
		{
			...
			// ======== panel3 ========
			{
				panel3.setLayout(new MigLayout("hidemode 3",
						// columns
						"[22,fill]" + "[fill]" + "[fill]" + "[34,fill]" + "[37,fill]" + "[fill]" + "[fill]",
						// rows
						"[]" + "[]" + "[]"));
